<template>
	<div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
		...
		<h1 class="page-header">
			...
			<input type="text" class="input_position" v-model="search_data" placeholder="请输入要查询的用户">
		</h1>
...
export default {
	...
	methods: {
		...
		clearMe() {
			this.$ajax
				.get('/delete_loginLogsByIds?_id=' + JSON.parse(localStorage.getItem('login')).user_id)
				.then(res => {
					this.reload();
				})
				.catch(err => {});
		...
		deleteOne(one) {
			this.$ajax
				.get('/delete_loginLogsById?_id=' + one)
				.then(res => {
					this.reload();
				})
				.catch(err => {});
		...
		initLoad(indexPage) {
			this.$ajax
				.get('/loginLog?indexPage=' + indexPage)
				.then(res => {
